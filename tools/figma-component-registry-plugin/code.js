...
function normalizeOptions(rawOptions) {
  ...
  return {
    ...
    fileKey: normalizeFileKey(options.fileKey),
  }
}
...
async function exportComponentData(roots, options) {
  ...
  return {
    ...
    source: createSourceMeta(options),
...
function createSourceMeta(options) {
  return compactObject({
    fileName: figma.root.name,
    fileKey: options.fileKey || getFileKey(),
    currentPage: figma.currentPage.name,
    scope: options.scope,
  })
}
...
function getFileKey() {
  return typeof figma.fileKey === 'string' && figma.fileKey ? figma.fileKey : undefined
}
