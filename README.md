...
通过清理无效、默认、隐藏或噪声字段，MCP Server 能够极大程度地压缩节点 JSON 体积。

以下以一个包含隐藏 Tab 页的 Tabs 节点（`node-id=3078-3670`）为例，对比 **Compact（紧凑）** 与 **Pretty（美化）** 两种格式的特点，以及 `visible:false` 过滤优化前后的实际收益：

| 维度 / 指标 | Compact JSON (紧凑格式) | Pretty JSON (美化格式) |
| :--- | :--- | :--- |
| **定义与特征** | 去除所有空格和换行，将 JSON 压缩为单行。 | 保留缩进（双空格）和换行，排版结构化。 |
| **适用场景** | 适合网络传输、API 交互、极致节省 Agent 上下文 Token 空间。 | 适合开发调试、控制台输出、Agent 直接阅读并理解节点树层级。 |
| **优缺点** | **优点**：体积极小。<br>**缺点**：人类和 Agent 极难直接阅读。 | **优点**：极易阅读，层级清晰。<br>**缺点**：体积较紧凑格式翻倍，消耗更多 Token。 |
| **Figma 原始大小** | 27.5 KB (完整响应) / 27.0 KB (仅节点数据) | 55.4 KB (完整响应) / 51.4 KB (仅节点数据) |
| **优化前 MCP 大小** | 16.2 KB | 29.6 KB |
| **优化后 MCP 大小** | **5.6 KB** (降幅 **65.53%**) | **9.8 KB** (降幅 **66.98%**) |
| **体积减少量** | **-10.6 KB** (-10,645 bytes) | **-19.8 KB** (-19,843 bytes) |
