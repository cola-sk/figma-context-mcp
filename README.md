...
通过清理无效、默认、隐藏或噪声字段，MCP Server 能够极大程度地压缩节点 JSON 体积。以下以一个包含隐藏 Tab 页的 Tabs 节点（`node-id=3078-3670`）为例，展示在引入 `visible:false` 子树整棵过滤前后的优化对比、格式差异及实际收益。

### JSON 格式对比与优化效果

| 格式 | 定义与特点 | 适用场景与优缺点 | Figma 原始大小<br>(完整/仅节点) | 优化前大小 | 优化后大小<br>(降幅) | 减少大小 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Compact JSON**<br>(紧凑格式) | 去除所有不必要的空格、换行和缩进，将 JSON 压缩为单行。 | **场景**：网络传输、API 交互、极致节省 Token。<br>**优点**：体积最小，极高节省 Token 消耗。<br>**缺点**：人类或 Agent 直接阅读和调试难度极高。 | 27,524 bytes<br>(仅节点: 26,989) | 16,244 bytes | **5,599 bytes**<br>(降幅 65.53%) | -10,645 bytes |
| **Pretty JSON**<br>(美化格式) | 带有结构化的缩进（如双空格）和换行排版。 | **场景**：开发调试、UI 展示、Agent 直接理解上下文结构。<br>**优点**：易读性极佳，层级关系一目了然，利于理解和生成代码。<br>**缺点**：体积翻倍，消耗更多 Token。 | 55,359 bytes<br>(仅节点: 51,449) | 29,627 bytes | **9,784 bytes**<br>(降幅 66.98%) | -19,843 bytes |

> [!NOTE]
> 引入 `visible:false` 优化后，生成的文档节点数已从 **17** 个大幅减少到 **5** 个。
